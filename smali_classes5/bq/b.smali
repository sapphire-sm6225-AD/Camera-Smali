.class public Lbq/b;
.super Ljava/lang/Object;
.source "SourceFile"


# annotations
.annotation runtime Ljava/lang/Deprecated;
.end annotation

.annotation build Lvp/a;
    threading = .enum Lvp/d;->b:Lvp/d;
.end annotation


# instance fields
.field public final a:Lyp/e;


# direct methods
.method public constructor <init>(Lyp/e;)V
    .locals 1

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    const-string v0, "Content length strategy"

    invoke-static {p1, v0}, Lkq/a;->j(Ljava/lang/Object;Ljava/lang/String;)Ljava/lang/Object;

    move-result-object p1

    check-cast p1, Lyp/e;

    iput-object p1, p0, Lbq/b;->a:Lyp/e;

    return-void
.end method


# virtual methods
.method public a(Leq/h;Lup/u;)Lup/o;
    .locals 1
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lup/q;,
            Ljava/io/IOException;
        }
    .end annotation

    const-string v0, "Session input buffer"

    invoke-static {p1, v0}, Lkq/a;->j(Ljava/lang/Object;Ljava/lang/String;)Ljava/lang/Object;

    const-string v0, "HTTP message"

    invoke-static {p2, v0}, Lkq/a;->j(Ljava/lang/Object;Ljava/lang/String;)Ljava/lang/Object;

    invoke-virtual {p0, p1, p2}, Lbq/b;->b(Leq/h;Lup/u;)Lyp/b;

    move-result-object p0

    return-object p0
.end method

.method public b(Leq/h;Lup/u;)Lyp/b;
    .locals 6
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Lup/q;,
            Ljava/io/IOException;
        }
    .end annotation

    new-instance v0, Lyp/b;

    invoke-direct {v0}, Lyp/b;-><init>()V

    iget-object p0, p0, Lbq/b;->a:Lyp/e;

    invoke-interface {p0, p2}, Lyp/e;->a(Lup/u;)J

    move-result-wide v1

    const-wide/16 v3, -0x2

    cmp-long p0, v1, v3

    const-wide/16 v3, -0x1

    if-nez p0, :cond_0

    const/4 p0, 0x1

    invoke-virtual {v0, p0}, Lyp/a;->u(Z)V

    invoke-virtual {v0, v3, v4}, Lyp/b;->j0(J)V

    new-instance p0, Lcq/e;

    invoke-direct {p0, p1}, Lcq/e;-><init>(Leq/h;)V

    invoke-virtual {v0, p0}, Lyp/b;->d0(Ljava/io/InputStream;)V

    goto :goto_0

    :cond_0
    cmp-long p0, v1, v3

    const/4 v5, 0x0

    if-nez p0, :cond_1

    invoke-virtual {v0, v5}, Lyp/a;->u(Z)V

    invoke-virtual {v0, v3, v4}, Lyp/b;->j0(J)V

    new-instance p0, Lcq/w;

    invoke-direct {p0, p1}, Lcq/w;-><init>(Leq/h;)V

    invoke-virtual {v0, p0}, Lyp/b;->d0(Ljava/io/InputStream;)V

    goto :goto_0

    :cond_1
    invoke-virtual {v0, v5}, Lyp/a;->u(Z)V

    invoke-virtual {v0, v1, v2}, Lyp/b;->j0(J)V

    new-instance p0, Lcq/g;

    invoke-direct {p0, p1, v1, v2}, Lcq/g;-><init>(Leq/h;J)V

    invoke-virtual {v0, p0}, Lyp/b;->d0(Ljava/io/InputStream;)V

    :goto_0
    const-string p0, "Content-Type"

    invoke-interface {p2, p0}, Lup/u;->B(Ljava/lang/String;)Lup/g;

    move-result-object p0

    if-eqz p0, :cond_2

    invoke-virtual {v0, p0}, Lyp/a;->N(Lup/g;)V

    :cond_2
    const-string p0, "Content-Encoding"

    invoke-interface {p2, p0}, Lup/u;->B(Ljava/lang/String;)Lup/g;

    move-result-object p0

    if-eqz p0, :cond_3

    invoke-virtual {v0, p0}, Lyp/a;->F(Lup/g;)V

    :cond_3
    return-object v0
.end method
